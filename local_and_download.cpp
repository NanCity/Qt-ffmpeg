...
Local_and_Download::Local_and_Download(QWidget* parent)
	...
	ui->tableWidget->setWindowFlags(Qt::CustomizeWindowHint | Qt::FramelessWindowHint);
...
void Local_and_Download::on_tableWidget_customContextMenuRequested(const QPoint& pos)
{
	...
	QAction* play = new QAction(QIcon(":/images/bottom/btn_play.png"), tr("Play"), menu);
	QAction* nextplay = new QAction(QIcon(":/images/bottom/btn_play.png"), tr("Next Play"), menu);
	...
	connect(play, &QAction::triggered, this, [=]() {
		...
		emit t_play(curItem->row());
		});

	//发送播放下一首信号
	connect(nextplay, &QAction::triggered, this, [=]() {
		QTableWidgetItem* curItem = ui->tableWidget->itemAt(pos);
		emit t_nextplay(curItem->row());
		});
