/*
 * @Author: your name
 * @Date: 2021-12-06 15:32:02
 * @LastEditTime: 2021-12-06 23:50:34
 * @LastEditors: 爱酱丶
 * @Description: 打开koroFileHeader查看配置 进行设置:
 * https://github.com/OBKoro1/koro1FileHeader/wiki/%E9%85%8D%E7%BD%AE
 * @FilePath: \Music\Decode.cpp
 */
#include "Decode.h"
...
void AudioDeCode::play(const QString &url) {
  this->url = url;
  DeCodeTag(url.toStdString().c_str());
...
QStringList AudioDeCode::DeCodeTag(const char *filename) {
  ...
  //   M_Format->streams[i]->codecpar->codec_type; if (avMediaType ==
  //   AVMEDIA_TYPE_AUDIO) {
  //     AudioStreamIndex = i; //查询到音频流
  //     break;
  //   }
  // }
  //开始解析音频流，key,value来储存解析出来的数据
  while (
  ...
  QImage AudioDeCode::Image(const char *) {
    // for (size_t i = 0; i <= AudioStreamIndex; ++i) {
    if (M_Format->streams[1]->disposition & AV_DISPOSITION_ATTACHED_PIC) {
      AVPacket pkt = M_Format->streams[1]->attached_pic;
  ...
  QString AudioDeCode::Duration() {
    ...
    return QString("%1 : %2").arg(mins).arg(cesc);
  }
