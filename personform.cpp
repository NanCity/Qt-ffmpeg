...
// http://localhost:3000/daily_signin
//签到
void PersonForm::on_btn_checkin_clicked() {
	 ...
	 */
	request = config->setCookies();
	QString url{ "http://localhost:3000/daily_signin?type=0" };
	request->setUrl(url);
	NetCheckin = manger->get(*request);

	connect(NetCheckin, &QNetworkReply::finished, this,
...
void PersonForm::on_btn_out_clicked() {
	QString url{ "http://localhost:3000/logout" };
	request->setUrl(url);
	NetOut = manger->get(*request);
	connect(NetOut, &QNetworkReply::finished, this, &PersonForm::on_logout);
}

//签到
void PersonForm::on_finshedCheckin() {
	if (NetCheckin->error() == QNetworkReply::NoError) {
		QJsonParseError errt_t{};
		QJsonDocument document = QJsonDocument::fromJson(NetCheckin->readAll(), &errt_t);
...
void PersonForm::on_logout() {
	if (NetOut->error() == QNetworkReply::NoError) {
		//删除配置文件
