...
Base::Base(QWidget *parent) { InitMedia(); }
...
//初始化TableWidget
void Base::InitTableWidget(QTableWidget *tab) {
  //设置无边框
...
void Base::DelTableWidgetRow(QTableWidget *tab) {
  for (int index = tab->rowCount(); index >= 0; --index) {
    tab->removeRow(index);
  }
}

void Base::SerachData(QTableWidget *table, QString search_data) {
  const int curRow = table->rowCount();
  if ("" == search_data) {
    ...
    QList<QTableWidgetItem *> item =
        table->findItems(search_data, Qt::MatchContains);
    for (int i = 0; i != curRow; i++) {
      //隐藏所有行
    ...
    if (!item.isEmpty()) {
      ...
      for (int i = 0; i != item.count(); i++) {
        table->setRowHidden(item.at(i)->row(), false);
      }
    }
  }
}
...
int TableMenu(QTableWidget *table) {
  ...
  QAction *play = new QAction(QIcon(":/images/Bottom/btn_stop_h.png"),
                              QObject::tr("播放"), table);
  Menu->addAction(play);
  Menu->show();
  return 1;
