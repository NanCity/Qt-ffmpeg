...
Login::Login(QWidget* parent) : QDialog(parent), ui(new Ui::Login)
{
	...
	//请求结束以后调用该槽
	connect(NetManager, &QNetworkAccessManager::finished, this, &Login::replyFinished);
	connect(ui->checkBox, &QCheckBox::stateChanged, this, &Login::checkBoxState);
	//connect(ui->line_phoneID, &QLineEdit::returnPressed, this, &Login::checkInput);
}
...
void Login::on_btn_signin_clicked()
{
	...
	static QRegExp regx_phone("^1([358][0-9]|4[579]|66|7[0135678]|9[89])[0-9]{8}$");
	checkInput(ui->line_phoneID->text(), regx_phone, 1);
	//验证密码 8-16位且必须为数字、大小写字母或符号中至少2种
	static QRegExp regx_pwd{ "^(?![\\d]+$)(?![a-zA-Z]+$)(?![^\\da-zA-Z]+$).{8,16}$" };
	...
	//以get方式发送一个网络请求
	NetManager->get(QNetworkRequest(QString("http://cloud-music.pl-fe.cn/login/cellphone?phone=%1&password=%2")
		.arg(phone).arg(psw)));
}
...
bool Login::checkInput(QString& str, const QRegExp regx, const int n)
{
	...
	case 1:
		...
		if (!(QValidator::Acceptable == res)) {
			if (QMessageBox::warning(this, tr("Warning"), tr("Please enter an 11-digit mobile phone\n")) == QMessageBox::Ok) {
...
Personal_information::Personal_information() :ID{}, UserName{},
BackgroundUrl{}, AvatarUrl{}, Fans{}, Follows{}, playlistCount{}{
}
